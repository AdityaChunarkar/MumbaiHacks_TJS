      ...
      <div className="flex-1 flex items-center justify-center px-4 py-8">
        <div className="w-full max-w-md bg-white rounded-lg shadow-lg p-8">
          ...
          <div className="mb-6 h-2 bg-gray-200 rounded-full overflow-hidden">
            <div className="h-full w-3/5 bg-primary rounded-full transition-all duration-500"></div>
          </div>
          ...
          <p className="text-center text-gray-600 text-sm mb-6">
            To give you the best investment recommendations, we need to know more about your financial situation and goals.
          </p>

          {/* Form */}
          <form id="form" onSubmit={handleSubmit} className="space-y-4">
            {[
              { name: 'incomeRange', label: 'Income range' },
              { name: 'investmentPreferences', label: 'Investment preferences' },
              { name: 'financialGoals', label: 'Financial goals' },
              { name: 'riskTolerance', label: 'Risk tolerance' }
            ].map((field) => (
              <div key={field.name}>
            ...
            <button 
              onClick={() => window.location.href = '/detailsTwo'} 
